  ...
  return (
    <main id="main-content" className="interior-page shell">
      <header className="about-masthead">
        <div><p className="eyebrow">Profile / 01</p><h1>Engineer by training.<br /><em>Systems thinker</em> by habit.</h1><p className="page-lede">I’m an AI engineer in Connecticut building at the intersection of models, software, and operational reality.</p></div>
        <HeadshotPlaceholder />
      </header>
      <section className="about-principles section-rule">
      ...
      <section className="timeline section-rule">
        ...
        {publicExperience.map((item) => <article className="timeline-row" key={`${item.period}-${item.role}`}><time>{item.period}</time><div><h2>{item.role}</h2><span>{item.organization}</span></div><div><p>{item.summary}</p><ul>{item.details.map((detail) => <li key={detail}>{detail}</li>)}</ul></div></article>)}
      </section>
